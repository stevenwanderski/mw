 ...
 */
function bx_css_stripped($match = array('modules/*'), $exceptions = NULL) {
 ...
 */
function bx_print_book_children($node) {
    ...
    if (isset($_GET['print']) && isset($_GET['book_recurse']) && !isset($semaphore)) {
      ...
      if (!empty($tree['below'])) {
        foreach ($tree['below'] as $link) {
          _bx_print_book_children($link, $child_pages, $zomglimit);
        }
      }
 ...
 */
function _bx_print_book_children($link, &$content, &$zomglimit, $limit = 500) {
  if ($zomglimit < $limit) {
    $zomglimit++;
    if (!empty($link['link']['nid'])) {
      ...
      if (!empty($link['below'])) {
        foreach ($link['below'] as $child) {
          _bx_print_book_children($child, $content);
        }
      }
    }
  }
...
function bx_preprocess_page(&$vars) {
  ...
  $attr['class'] .= ' bx'; // Add the bx class so that we can avoid using the 'body' selector
 ...
 */
function bx_preprocess_box(&$vars) {
  $attr = array();
  $attr['class'] = "box";
  $vars['attr'] = $attr;
  $vars['hook'] = 'box';
...
function bx_preprocess_node(&$vars) {
  ...
  if (isset($_GET['print'])) {
    $vars['post_object'] = bx_print_book_children($vars['node']);
  }
}
 ...
 */
function bx_preprocess_form_element(&$vars) {
  $element = $vars['element'];
 ...
 */
function bx_preprocess_print_header(&$vars) {
  $vars = array(
    'base_path' => base_path(),
 ...
 */
function bx_menu_local_tasks($type = '') {
  if ($primary = menu_primary_local_tasks()) {
    $primary = "<ul class='tabs primary'>{$primary}</ul>";
  }
  if ($secondary = menu_secondary_local_tasks()) {
    $secondary = "<ul class='tabs secondary'>$secondary</ul>";
  }
  switch ($type) {
    case 'primary':
      return $primary;
 ...
 */
function bx_pager($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 9) {
 ...
 */
function bx_pager_list($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 9) {
 ...
 */
function bx_pager_link($text, $page_new, $element, $parameters = array(), $attributes = array()) {
 ...
 */
function bx_views_mini_pager($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 9) {
